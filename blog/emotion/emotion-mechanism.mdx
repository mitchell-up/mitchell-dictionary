...
### 주의깊게 볼 패키지
이모션은 스타일을 생성하고 최적화하기 위한 많은 패키지들을 가지고 있습니다. 그러나 전체적인 흐름을 이해하기 위해 다음의 패키지들의 코드를 이해하면서 넘어가게 됩니다.  
...
7. stylis로 스타일을 CSSRule로 컴파일
8. 컴파일된 CSSrule을 HTML문서에 삽입
...
복잡했던 스타일 직렬화 과정을 거쳐서 생성될 `<Insertion/>` 컴포넌트에서는 직렬화된 스타일을 캐시에 등록하고 삽입하는 과정을 처리하게됩니다.
```tsx title="emotion/packages/react/src/emotion-element.js"
...
### registerStyles
전달받은 캐시의 key와 직렬화된 스타일의 name으로 className을 생성하고, 그 값을 key로 현재의 직렬화된 스타일이 캐시에 등록되어 있지 않으면 캐시에 등록합니다.  
[소스코드 바로가기](https://github.com/emotion-js/emotion/blob/main/packages/utils/src/index.js#L23)

### insertStyles
`insertStyles`에서는 캐시를 통해 주입되지 않은 스타일들을 주입합니다.
```tsx
...
### stylis
stylis는 CSS preprocessor로 CSSRule로 컴파일 하는 라이브러리입니다. Emotion은 내부적으로 stylis를 통해 직렬화된 스타일을 처리합니다. 이 과정에서 직렬화된 스타일은 CSSRule로 파싱되고, Vendor Prefix를 추가하고 별도의 최적화처리를 거치게 됩니다.
```tsx
...
```
위 stylis라는 함수 미들웨어로 등록된 플러그인의 내용중 finalizingPlugins을 살펴보면 스타일을 주입하는 코드를 확인할 수 있습니다. 해당 코드는 `StyleSheet.insert`이고 이를 통해 브라우저에 스타일이 삽입됩니다.

---

## 정리
복잡한 스타일 직렬화, 캐시등록 및 삽입, 컴파일 과정을 거쳐서 최종적으로 StyleSheet에 스타일이 삽입됩니다.

그 과정에서 만들어지는 className이 본래의 컴포넌트로 전달되며, 본래의 컴포넌트는 등록된 스타일과 함께 렌더링 될 수 있게 됩니다.
